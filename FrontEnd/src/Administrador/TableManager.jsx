...
export const STATIC_TABLE_DEFS = {
    ...
    configuracaousuario: {
        ...
        fields: [
            { name: 'usuario_id', label: 'ID do Usuário', type: 'number', required: true, readOnly: true },
...
const TableManager = ({ table }) => {
    ...
    // primary key field for current table (fallback to first column when no "id")
    const pkField = useMemo(() => {
        if (!tableInfo || !tableInfo.columns) return 'id';
        return tableInfo.columns.includes('id') ? 'id' : tableInfo.columns[0];
    }, [tableInfo]);

    /* // Static table definitions – keys must match backend model names exactly (singular forms)
    // local copy is commented out; using exported version above
    const STATIC_TABLE_DEFS = {
    user: {
        title: 'Usuários',
        icon: '👥',
        columns: ['id', 'nome', 'email', 'telefone', 'isAdmin'],
    ...
    participante_torneio: {
        ...
        displayColumns: ['ID', 'Torneio ID', 'Usuário ID', 'Status', 'Pontuação'],
        fields: [
            { name: 'torneio_id', label: 'ID do Torneio', type: 'number', required: true },
            { name: 'usuario_id', label: 'ID do Usuário', type: 'number', required: true },
            { name: 'status', label: 'Status', type: 'select', options: ['pendente', 'confirmado', 'removido', 'desclassificado'] },
            { name: 'pontuacao', label: 'Pontuação', type: 'number' },
    ...
    const handleModalSubmit = async (formData) => {
        ...
        try {
            if (modalMode === 'create') {
                ...
                if (!idValue && modalMode !== 'create') {
                    setError('Identificador inválido para operação');
                    return;
                }
                if (modalMode === 'edit') {
