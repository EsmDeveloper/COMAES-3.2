/**
 * QuestoesColaboradoresTab.jsx (REFATORADO)
 * Aba para visualizar questões aprovadas de colaboradores
 * Separado em: Blocos Aprovados + Questões Solo Aprovadas
 */

import { useState, useEffect, useCallback, useReducer } from 'react';
import axios from 'axios';
import {
  Search, Filter, ChevronDown, BookOpen, Trash2, Eye,
  AlertCircle, RefreshCw, X, FileText, Layers
} from 'lucide-react';
import {
  DificuldadeBadge,
  DisciplinaBadge,
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_BLOCOS_APROVADOS':
      return { ...state, blocosAprovados: action.payload };
    case 'SET_QUESTOES_SOLO_APROVADAS':
      return { ...state, questoesSoloAprovadas: action.payload };
    case 'DELETE_BLOCO':
      return { ...state, blocosAprovados: state.blocosAprovados.filter(b => b.id !== action.payload) };
    case 'DELETE_QUESTAO':
      return { ...state, questoesSoloAprovadas: state.questoesSoloAprovadas.filter(q => q.id !== action.payload) };
    case 'UPDATE_FILTRO':
...
export default function QuestoesColaboradoresTab() {
  ...
  const carregarBlocosAprovados = useCallback(async () => {
    ...
    try {
      ...
      const blocos = response.data?.dados?.blocos || response.data?.blocos || [];
      dispatch({ type: 'SET_BLOCOS_APROVADOS', payload: blocos });
    } catch (err) {
      console.error('Erro ao carregar blocos:', err);
      dispatch({ type: 'SET_ERROR', payload: 'Erro ao carregar blocos' });
    } finally {
      dispatch({ type: 'SET_LOADING_BLOCOS', payload: false });
    }
  }, [state.filtros.disciplina, token]);

  // Carregar questões solo aprovadas
  const carregarQuestoesSoloAprovadas = useCallback(async () => {
    dispatch({ type: 'SET_LOADING_QUESTOES', payload: true });
    try {
      const params = { status_aprovacao: 'aprovada', sem_bloco: true };
      if (state.filtros.disciplina) params.disciplina = state.filtros.disciplina;

      const response = await axios.get(`${apiBaseUrl}/api/questoes`, {
        params,
        headers: { Authorization: `Bearer ${token}` }
      });

      const questoes = response.data?.dados?.questoes || response.data?.questoes || [];
      dispatch({ type: 'SET_QUESTOES_SOLO_APROVADAS', payload: questoes });
    } catch (err) {
  ...
  }, [carregarBlocosAprovados, carregarQuestoesSoloAprovadas]);

  // Deletar bloco
  const handleDeletarBloco = async (id) => {
    setActionLoading(id);
    try {
      await axios.delete(`${apiBaseUrl}/api/blocos-colaboradores/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      mostrarToast('Bloco deletado com sucesso!', 'success');
      dispatch({ type: 'DELETE_BLOCO', payload: id });
      setDeleteModalOpen(false);
      setItemParaDeletar(null);
    } catch (err) {
      mostrarToast(err.response?.data?.mensagem || 'Erro ao deletar', 'error');
    } finally {
      setActionLoading(null);
    }
  };

  // Deletar questão solo
  const handleDeletarQuestao = async (id) => {
    setActionLoading(id);
    try {
      await axios.delete(`${apiBaseUrl}/api/questoes/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      mostrarToast('Questão deletada com sucesso!', 'success');
      dispatch({ type: 'DELETE_QUESTAO', payload: id });
      setDeleteModalOpen(false);
      setItemParaDeletar(null);
    } catch (err) {
      mostrarToast(err.response?.data?.mensagem || 'Erro ao deletar', 'error');
  ...
  const openDeleteModal = (item, tipo) => {
    setItemParaDeletar({ ...item, tipo });
  ...
  // Filtrar blocos
  const blocosFiltrados = state.blocosAprovados.filter(b => {
  ...
  const questoesFiltradas = state.questoesSoloAprovadas.filter(q => {
    if (!state.filtros.busca) return true;
    const buscaLower = state.filtros.busca.toLowerCase();
    return (
    ...
    <div className="p-6 space-y-8">
      ...
      <div>
        <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2 mb-2">
          ...
          Questões Aprovadas dos Colaboradores
        </h2>
        <p className="text-slate-600">
          Visualize e gerencie questões e blocos aprovados criados pelos colaboradores
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* SEÇÃO 1: BLOCOS APROVADOS */}
        <div className="space-y-4">
          <div className="flex items-center gap-2 sticky top-0 z-10 bg-white pb-2">
            <Layers className="w-5 h-5 text-blue-600" />
            <h3 className="text-lg font-bold text-slate-800">Blocos Aprovados</h3>
            ...
            {blocosFiltrados.map((bloco) => (
              <div key={bloco.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
                  ...
                  <div className="flex-1" />
                  <button
                    onClick={() => openDeleteModal(bloco, 'bloco')}
        ...
        {/* SEÇÃO 2: QUESTÕES SOLO APROVADAS */}
        <div className="space-y-4">
          <div className="flex items-center gap-2 sticky top-0 z-10 bg-white pb-2">
            <FileText className="w-5 h-5 text-indigo-600" />
            <h3 className="text-lg font-bold text-slate-800">Questões Solo Aprovadas</h3>
            <span className="ml-auto text-sm font-semibold text-slate-600 bg-indigo-50 px-3 py-1 rounded-lg">
              {questoesFiltradas.length}
            </span>
          </div>

        {state.loadingQuestoes && state.questoesSoloAprovadas.length === 0 && (
          <div className="bg-white rounded-xl p-8 text-center">
        ...
        {!state.loadingQuestoes && questoesFiltradas.length === 0 && (
          <div className="bg-white rounded-xl p-8 text-center border border-slate-200">
            ...
            <p className="text-slate-600">Nenhuma questão solo aprovada</p>
          </div>
        )}
              ...
              return (
                <div key={questao.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
                    ...
                    <div className="flex-1" />
                    <button
                      onClick={() => openDeleteModal(questao, 'questao')}
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (itemParaDeletar?.tipo === 'bloco') {
            handleDeletarBloco(itemParaDeletar.id);
          } else {
            handleDeletarQuestao(itemParaDeletar.id);
          }
        }}
        titulo={itemParaDeletar?.tipo === 'bloco' ? 'Deletar Bloco' : 'Deletar Questão'}
        mensagem={`Tem certeza que deseja deletar ${itemParaDeletar?.tipo === 'bloco' ? 'o bloco' : 'a questão'} "${itemParaDeletar?.titulo}"? Esta ação não pode ser desfeita.`}
        buttonText="Deletar"
        buttonVariant="red"
        loading={actionLoading === itemParaDeletar?.id}
      />
