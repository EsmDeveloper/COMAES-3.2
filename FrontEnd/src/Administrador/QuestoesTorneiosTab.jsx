  ...
  const handleAgruparEmBloco = async (blocoId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(`[MEDAL] Mensagem completa:`, JSON.stringify(errorData, null, 2));
                    ...
                    <tbody className="divide-y divide-blue-100">
                      {filteredQuestoes.map(questao => (
                        <tr key={questao.id} className="hover:bg-blue-50 transition-colors">
                          <td className="px-4 py-3 text-sm font-medium text-gray-900">{questao.titulo}</td>
                          <td className="px-4 py-3 text-sm">
                            <span className="px-2 py-1 bg-purple-100 text-purple-700 text-xs font-semibold rounded">
                              {questao.disciplina?.toUpperCase()}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm">
