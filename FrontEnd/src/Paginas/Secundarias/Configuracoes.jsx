/**
 * Configuracoes.jsx
 * P�gina de configura���es pessoais — experi��ncia de uso da conta.
 *
 * CONT�M:  altera��o de senha, seguran�a, idioma, notifica���es, privacidade
 * N�O CONT�M: nome, foto, biografia, escola, telefone → est�o no Perfil (/perfil)
 */
...
function AlterarSenhaForm({ token, onSuccess, onError }) {
  ...
  const validate = () => {
    const e = {};
    if (!form.senhaAtual) e.senhaAtual = 'Obrigat�rio';
    if (!form.novaSenha)  e.novaSenha  = 'Obrigat�rio';
    else {
      const r = validatePassword(form.novaSenha);
      if (!r.valid) e.novaSenha = r.error;
    }
    if (!form.confirmar) e.confirmar = 'Obrigat�rio';
    else if (form.novaSenha !== form.confirmar) e.confirmar = 'As senhas n�o coincidem';
    return e;
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="py-4 flex flex-col gap-4">
      ...
      <div>
        ...
        <PasswordInput field="novaSenha" placeholder="M�nimo 8 caracteres" />
        <p className="text-xs text-gray-400 mt-1">Deve conter mai��sculas, min��sculas, n��mero e s�mbolo.</p>
      </div>
      <div>
...
export default function Configuracoes() {
  ...
  useEffect(() => {
    if (!user) return;
    fetch(`${API}/api/usuarios/${user.id}/configuracao`, {
  ...
  /* ── Persist helper ─────────────────────────────────────────
     O backend faz merge profundo — s� enviar a sec��o alterada.
     Estrutura: { seguranca: {...} } | { tema: '...' } | etc.
  ─────────────────────────────────────────────────────────── */
  const persist = async (patch) => {
    try {
      const res = await fetch(`${API}/api/usuarios/${user.id}/configuracao`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      ...
      showToast('success', 'Prefer��ncia guardada.');
    } catch (err) {
  ...
  const toggleSave = (section, setter, key) => (val) => {
    setter(s => {
      ...
      // Enviar apenas a sec��o alterada — backend faz merge
  ...
  const panels = {
    ...
    seguranca: (
      <div className="flex flex-col gap-4">
        ...
        {/* Verifica��o */}
        <SectionCard title="Verifica��o" desc="Camadas adicionais de seguran�a para a sua conta.">
          <ToggleRow
            label="Autentica��o em dois fatores"
            desc="Exige um c�digo adicional ao fazer login."
        ...
        {/* Sess��es */}
        <SectionCard title="Sess��es Ativas" desc="Dispositivos com acesso activo �� sua conta.">
          <div className="py-4 border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-0.5">Sess�o atual</p>
              </div>
          ...
          <div className="py-4 flex items-center justify-between">
            ...
            >
              <LogOut size={14} /> Terminar sess�o
            </button>
            <button
              onClick={() => showToast('success', 'Outras sess��es encerradas.')}
              className="text-xs text-gray-500 hover:text-gray-700 underline"
            >
              Encerrar outras sess��es
            </button>
          </div>
        </SectionCard>
      </div>
    ...
    preferencias: (
      <div className="flex flex-col gap-4">
        <SectionCard title="Idioma" desc="Escolha o idioma de apresenta��o da plataforma.">
          <div className="py-4">
            ...
            >
              <option value="pt-PT">Portugu��s (Angola)</option>
    ...
    privacidade: (
      <SectionCard title="Privacidade" desc="Controle o que outros utilizadores podem ver sobre si.">
        ...
        />
        <ToggleRow
          label="Exibir posi��o no ranking"
          desc="Mostra a sua posi��o no ranking p��blico da plataforma."
          active={priv.mostrarRanking}
          onChange={toggleSave('privacidade', setPriv, 'mostrarRanking')}
        />
        <ToggleRow
          label="Exibir certificados"
          desc="Torna os seus certificados vis�veis no perfil p��blico."
        ...
        <ToggleRow
          ...
          desc="Mostra participa���es e conquistas recentes no perfil."
    ...
    /* ── Notifica���es ── */
    notificacoes: (
      <SectionCard title="Notifica���es" desc="Escolha quais alertas deseja receber.">
        <ToggleRow
          label="Notifica���es por e-mail"
          desc="Receba notifica���es gerais no seu e-mail."
          active={notif.email}
          onChange={toggleSave('notificacoes', setNotif, 'email')}
        />
        <ToggleRow
          label="Competi���es e torneios"
  ...
  return (
    <Layout>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 pb-20">
        ...
        {/* Page header */}
        <div className="mb-7">
          <h1 className="text-2xl font-bold text-gray-900">Configura���es</h1>
          <p className="text-sm text-gray-500 mt-1">
            Gerencie a sua seguran�a, prefer��ncias e privacidade.
          </p>
        </div>
