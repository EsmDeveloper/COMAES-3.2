...
function Cadastro() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || `http://${window.location.hostname}:3000`}/auth/registro`, {
        method: 'POST',
      ...
      const loginRes = await fetch(`${import.meta.env.VITE_API_BASE_URL || `http://${window.location.hostname}:3000`}/auth/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
