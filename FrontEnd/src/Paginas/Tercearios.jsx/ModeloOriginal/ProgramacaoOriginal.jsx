...
export default function ProgramacaoOriginal() {
  ...
  const buscarQuestoes = async (torneioId) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || `http://${window.location.hostname}:3000`}/torneios/${torneioId}/questoes/programacao`);
                ...
                ranking.map((participanteRank) => {
                  ...
                  <tr key={participanteRank.id}
                    className={`border-b transition-colors ${
                      isMe
                        ? 'bg-purple-50 border-l-4 border-l-purple-500 border-b-purple-100'
                      ...
                      <div className="flex items-center gap-2">
                        {participanteRank.usuario?.imagem ? (
                          <img 
                            ...
                            className={`w-7 h-7 rounded-full object-cover ${isMe ? 'ring-2 ring-purple-500' : ''}`}
                          />
                        ) : (
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs ${isMe ? 'bg-purple-600 ring-2 ring-purple-400' : 'bg-purple-500'}`}>
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          {isMe && (
                            <span className="text-[10px] font-bold text-purple-500 leading-none">● você</span>
                          )}
                        </div>
                      </div>
                    </td>
                    <td className={`px-2 py-2 font-semibold ${isMe ? 'text-purple-700' : 'text-purple-600'}`}>
        ...
        >
          ...
          {questoesFiltradas.length === 0 ? (
            ...
            <>
              ...
              {/* BOTÕES DE CONTROLE - APENAS EXECUTAR CÓDIGO */}
