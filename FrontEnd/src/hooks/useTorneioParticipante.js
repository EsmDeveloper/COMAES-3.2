 ...
 *
 * Hook partilhado pelas três páginas de torneio (Matemática, Inglês, Programação).
 *
 * Garante:
 ...
 *  4. Posições sempre calculadas localmente — nunca null/0/9999
 *  5. Compatibilidade com disciplinas acentuadas (Matemática, Inglês, Programação)
 */
...
function normalizarDisciplina(raw) {
  ...
  if (s === 'ingles' || s === 'lingua inglesa') return 'Inglês';
  return raw.charAt(0).toUpperCase() + raw.slice(1);
}

// ---------------------------------------------------------------------------
// Cálculo de posições locais — nunca devolve null/0/9999
// ---------------------------------------------------------------------------
...
export default function useTorneioParticipante({ disciplina, disciplinaSlug, user, token }) {
  ...
  // ── disciplina normalizada (usada para comparar eventos do socket) ─────────
  const disciplinaNorm = normalizarDisciplina(disciplina);

  // ── ordenar + calcular posições ────────────────────────────────────────────
  ...
  // ── adicionar participante ao ranking local imediatamente ──────────────────
  ...
  const registrarParticipante = useCallback(async (userId, torneioId) => {
    ...
    try {
      ...
      // Rebuscar ranking do servidor para garantir consistência
      await buscarRanking();
    } catch (err) {
  ...
  // ── buscar dados do utilizador (ou registar se não existir) ───────────────
  ...
  useEffect(() => {
    ...
    // Polling a cada 15 s — garante sincronização mesmo sem socket
  ...
  // ── actualizar participante após avaliação ─────────────────────────────────
