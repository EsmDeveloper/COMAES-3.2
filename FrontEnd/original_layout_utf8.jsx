...
export default function Layout({ children }) {
  ...
  const menuItems = [
    ...
    { icon: <FaBullhorn />, text: "Portal de Not├¡cias", link: "/portal-de-noticias" },
    { icon: <FaChartLine />, text: "Dashboard", link: "/painel" },
    { icon: <FaUserCircle />, text: "Perfil do Usu├írio", link: "/perfil" },
    { icon: <FaCogs />, text: "Configura├º├Áes", link: "/configuracoes" },
    { icon: <FaInfoCircle />, text: "Sobre n├│s", link: "/sobre-nos" },
    ...
    const fetchNotificationCount = async () => {
      if (user && user.id) {
        try {
          ...
          console.error("Erro ao buscar contagem de notifica├º├Áes:", error);
        }
      } else {
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            {/* Rodap├® da sidebar - info do usu├írio */}
            {user && (
              <div className="px-4 py-4 border-t border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{user.username || user.name || "Usu├írio"}</p>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  <span className="hidden sm:block text-sm font-medium">
                    {user.username || user.name || "Usu├írio"}
                  </span>
                  {user.avatar
      ...
      <footer className="bg-gray-900 text-gray-300 mt-auto w-full">
        <div className="max-w-7xl mx-auto px-6 py-8 sm:px-8 flex flex-col items-center gap-5">
          <div className="flex flex-wrap justify-center gap-x-5 gap-y-2 text-sm">
            ...
            <Link to="/politica-de-uso" className="text-gray-400 hover:text-blue-500 transition-colors duration-150">Pol├¡tica de Uso</Link>
            <Link to="/privacidade" className="text-gray-400 hover:text-blue-500 transition-colors duration-150">Privacidade</Link>
            <Link to="/seguranca" className="text-gray-400 hover:text-blue-500 transition-colors duration-150">Seguran├ºa</Link>
          </div>
          ...
          <p className="text-xs text-gray-500 text-center">
            &copy; 2025 Comaes ÔÇö Plataforma de Competi├º├Áes Educativas Online. Todos os direitos reservados.
          </p>
        </div>
      </footer>
    </div>
